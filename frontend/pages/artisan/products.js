...
export default function ArtisanProducts() {
  ...
  useEffect(() => {
    if (isAuthenticated && isArtisan) fetchProducts();
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/api/products?artisan=${user.id}`);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-orange-600">Mis Productos</h1>
      {loading ? (
        <div className="text-center py-12">Cargando productos...</div>
      ) : products.length === 0 ? (
        <div className="text-center py-12 text-gray-500">No tienes productos registrados.</div>
      ) : (
